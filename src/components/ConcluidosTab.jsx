...
function matchesSearch(report, search) {
  ...
  return [
    report.pacienteNome,
    report.sintomaNome,
    report.mensagemFinal,
    report.cid,
    formatDate(report.dataRelatorio),
    formatDate(report.impressoEm)
...
export function ConcluidosTab({ hospital, user }) {
  ...
  async function handleReprintSelected() {
    if (!hospital?.id || selectedReports.length === 0) {
      return
    }
    ...
    <section className="rounded-[1.5rem] border border-ink/10 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          ...
          disabled={carregando || selectedReports.length === 0}
      ...
      <div className="mt-4">
        <input
          ...
          placeholder="Pesquisar por paciente, sintoma, CID, mensagem ou data"
        />
      </div>
        ...
        {concluidos.length === 0 ? (
          ...
          <div key={report.id} className="border-t border-ink/10 px-4 py-4 text-sm sm:grid sm:grid-cols-[52px_1fr_1fr_112px] sm:items-center sm:gap-3">
            <div className="mb-3 flex items-center gap-3 sm:mb-0">
              <input
                type="checkbox"
                checked={selecionados.includes(report.id)}
                onChange={() => toggleSelecionado(report.id)}
            ...
            <span className="mt-2 block font-semibold text-ink/70 sm:mt-0">{report.sintomaNome || "Sem sintoma"}</span>
